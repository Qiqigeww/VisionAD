...
# 相比较于 dade_mvtec_test.py , 从 one-for-one 改成了 one-for-all 的模式;
...
import argparse
from utils import evaluation_batch, WarmCosineScheduler, FocalBCE, DiceLossBatch, evaluation_batch_1
...
def train():
    ...
    # encoder_name = 'aimv2-large-patch14-448'
    # encoder_name = 'aimv2-huge-patch14-448'
    # encoder_name = 'aimv2-1B-patch14-448'
    # encoder_name = 'aimv2-3B-patch14-448'
    
    # encoder_name = 'aimv2-large-patch14-336'
    # encoder_name = 'aimv2-huge-patch14-336'
    # encoder_name = 'aimv2-1B-patch14-336'
    # encoder_name = 'aimv2-3B-patch14-336'
    
    # encoder_name = 'aimv2-large-patch14-224'
    ...
    support_data_all = {}
    for item, support_data in zip(test_item_list, support_data_list):
        ...
        supports, _ = next(iter(support_loader))
        # supports = supports.to(device)
        # supports = supports.unsqueeze(dim=0).to(device)
        support_data_all[item] = supports
    # support_data_all = torch.cat(support_data_all, dim=1)

    for item, test_data in zip(test_item_list, test_data_list):
        test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False,
                                                      num_workers=0)#4

        results = evaluation_batch_1(model, test_dataloader, support_data_all, device, max_ratio=0.01,
                                   resize_mask=256)
