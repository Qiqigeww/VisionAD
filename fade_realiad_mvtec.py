...
from models.uad import FADE, FADEv4, ConditionHead
...
def train():
    ...
    train_transform = get_alb_transforms(image_size, crop_size)
    train_data_list = []
    """Train Data"""
    ...
    head = nn.Sequential(nn.Linear(embed_dim * 3, 512), nn.GELU(), nn.Linear(512, 1))
    # head = ConditionHead(embed_dim, 512)

    model = FADEv4(encoder=encoder, head=head,
                   interested_layers=interested_layers)
    model = model.to(device)

    optimizer = torch.optim.AdamW([{'params': model.head.parameters()}],
    ...
    for epoch in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        for bad_img, bad_gt, good_img, good_gt, support_img in train_dataloader:
            ...
            amap, pred = model(query_img, support_img)
            ...
            optimizer.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm(head.parameters(), max_norm=0.1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str,
                        default='fadev4_realiad_mvtec_dinov2br_bce5_saug_hl2_it20k_adam1e4_w2kcos_b8_s1')
