...
import logging
import os
from models import models_synclr, models_clip
_logger = logging.getLogger(__name__)

_WEIGHTS_DIR = "./Dinomaly/backbones/weights"
...
def load(name):
    ...
    if 'aimv2' not in name:
        arch, patchsize = name.split("_")[-2], name.split("_")[-1]
        if 'dinov2reg' not in name:
    ...
    if "dino" in name:
        if "v2" in name:
            if "reg" in name:
                ...
                if arch == "base":
                    ...
                    ckpt_pth = os.path.join(_WEIGHTS_DIR, "dinov2_vitg14_reg4_pretrain.pth")
                else:
    ...
    elif "proteus" in name:
        if "DINOV2" in name:
            model = vision_transformer_dinov2.__dict__[f'vit_{arch}'](patch_size=int(patchsize), img_size=224,
                                                                          block_chunks=0, init_values=1e-05,
                                                                          interpolate_antialias=False,
                                                                        interpolate_offset=0.1)
            if arch == "base":
                ckpt_pth = os.path.join(_WEIGHTS_DIR, "ViT_B_proteus_DINOV2.pth")
            elif arch == "small":
                ckpt_pth = os.path.join(_WEIGHTS_DIR, "ViT_S_proteus_DINOV2.pth")
            elif arch == "large":
                ckpt_pth = os.path.join(_WEIGHTS_DIR, "ViT_L_proteus_DINOV2.pth")
            else:
                raise ValueError("Invalid type of architecture. It must be either 'small' or 'base' or 'large.")
        elif "CLIP" in name:
            if arch == "base":
                model = models_clip.__dict__[f'vit_{arch}']()           
                ckpt_pth = os.path.join(_WEIGHTS_DIR, "ViT_B_proteus_CLIP.pth")
        elif "SynCLR" in name:
            if arch == "base":
                model = models_synclr.__dict__[f'vit_{arch}'](patch_size=int(patchsize), num_classes=0, mask_style='ibot')
                ckpt_pth = os.path.join(_WEIGHTS_DIR, "ViT_B_proteus_SynCLR.pth")
        state_dict = dinov2_utils.load_pretrained_weights_proteus(model, ckpt_pth, "teacher")

    elif "aimv2" in name:
        from ml_aim.aim_v2.aim.v2.torch import models
        from safetensors.torch import load_file
        
        ckpt_pth = os.path.join(_WEIGHTS_DIR, f"{name}-model.safetensors")
        
        prefix, model_size, _, img_size = name.split("-")
        model_func, img_size = f"{prefix}_{model_size}", int(img_size)
        ...
        if arch == "base":
            ckpt_pth = os.path.join(_WEIGHTS_DIR, "synclr_vit_b_16.pth")
            model = models_synclr.__dict__[f'vit_{arch}'](patch_size=int(patchsize), teacher_path=ckpt_pth, num_classes=0, mask_style='ibot')
            return model
    if "digpt" in name:
        if arch == 'base':
